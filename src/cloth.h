...
struct Cloth {
  Cloth() {}
  Cloth(double width, double height, float thickness);
  ~Cloth();
  ...
  int findNearest(int range, int pm_i, int pm_j, int target);
  int findFarthest(int pm_i, int pm_j, int target);
  ...
  //jio added cloth component
  map<int,int> pixel_indices;
