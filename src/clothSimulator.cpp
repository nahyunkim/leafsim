...
void ClothSimulator::loadCloths(vector<Cloth *> *cloths) { 
  this->cloths = cloths; 
  this->cloths_vector = *cloths;
}

void ClothSimulator::loadClothParameters(vector<ClothParameters *> *cps) { 
  this->cps = cps;
  this->cps_vector = *cps;
}
...
void ClothSimulator::init() {
  ...
  canonical_view_distance = max(cloths_vector[0]->width, cloths_vector[0]->height) * 0.9;
...
void ClothSimulator::drawContents() {
  ...
  if (!is_paused) {
    vector<Vector3D> external_accelerations = {gravity, wind_amplitude * (sin(current_step / wind_interval)+1)};
    current_step += 1.0 / frames_per_sec;

    for (int i = 0; i < simulation_steps; i++) {
      for (int j = 0; j < cloths_vector.size(); j++) {
        cloths_vector[j]->simulate(frames_per_sec, simulation_steps, cps_vector[j], external_accelerations, collision_objects);
      }
    }
  }
...
void ClothSimulator::drawWireframe(GLShader &shader) {
  for (int i = 0; i < cloths_vector.size(); i++) {
    int num_structural_springs =
        2 * (cloths_vector)[i]->num_width_points * (cloths_vector)[i]->num_height_points -
        cloths_vector[i]->num_width_points - cloths_vector[i]->num_height_points;
    int num_shear_springs =
        2 * (cloths_vector[i]->num_width_points - 1) * (cloths_vector[i]->num_height_points - 1);
    int num_bending_springs = num_structural_springs - cloths_vector[i]->num_width_points -
                              cloths_vector[i]->num_height_points;

    int num_springs = cps_vector[i]->enable_structural_constraints * num_structural_springs +
                      cps_vector[i]->enable_shearing_constraints * num_shear_springs +
                      cps_vector[i]->enable_bending_constraints * num_bending_springs;
    ...
    for (int j = 0; j < cloths_vector[i]->springs.size(); j++) {
      Spring s = cloths_vector[i]->springs[j];

      if ((s.spring_type == STRUCTURAL && !cps_vector[i]->enable_structural_constraints) ||
          (s.spring_type == SHEARING && !cps_vector[i]->enable_shearing_constraints) ||
          (s.spring_type == BENDING && !cps_vector[i]->enable_bending_constraints)) {
        continue;
      }
...
void ClothSimulator::drawNormals(GLShader &shader) {
  for (int i = 0; i < cloths_vector.size(); i++) {
    int num_tris = cloths_vector[i]->clothMesh->triangles.size();

    MatrixXf positions(3, num_tris * 3);
    MatrixXf normals(3, num_tris * 3);

    for (int j = 0; j < num_tris; j++) {
      Triangle *tri = cloths_vector[i]->clothMesh->triangles[j];
      ...
      positions.col(j * 3) << p1.x, p1.y, p1.z;
      positions.col(j * 3 + 1) << p2.x, p2.y, p2.z;
      positions.col(j * 3 + 2) << p3.x, p3.y, p3.z;

      normals.col(j * 3) << n1.x, n1.y, n1.z;
      normals.col(j * 3 + 1) << n2.x, n2.y, n2.z;
      normals.col(j * 3 + 2) << n3.x, n3.y, n3.z;
    }
...
void ClothSimulator::initGUI(Screen *screen) {
...

  {
    ...
    b->setPushed(cps_vector[0]->enable_structural_constraints);
    b->setFontSize(14);
    b->setChangeCallback(
        [this](bool state) { for (ClothParameters *cp : cps_vector) cp->enable_structural_constraints = state; });

    b = new Button(window, "shearing");
    b->setFlags(Button::ToggleButton);
    b->setPushed(cps_vector[0]->enable_shearing_constraints);
    b->setFontSize(14);
    b->setChangeCallback(
        [this](bool state) { for (ClothParameters *cp : cps_vector) cp->enable_shearing_constraints = state; });

    b = new Button(window, "bending");
    b->setFlags(Button::ToggleButton);
    b->setPushed(cps_vector[0]->enable_bending_constraints);
    b->setFontSize(14);
    b->setChangeCallback(
        [this](bool state) { for (ClothParameters *cp : cps_vector) cp->enable_bending_constraints = state; });
  }
...

  {
    ...
    fb->setFontSize(14);
    fb->setValue(cps_vector[0]->density / 10);
    fb->setUnits("g/cm^2");
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { for (ClothParameters *cp : cps_vector) cp->density = (double)(value * 10); });
    ...
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(cps_vector[0]->ks);
    fb->setUnits("N/m");
    fb->setSpinnable(true);
    fb->setMinValue(0);
    fb->setCallback([this](float value) { for (ClothParameters *cp : cps_vector) cp->ks = value; });
  }
...

  {
    ...
    slider->setValue(cps_vector[0]->damping);
    slider->setFixedWidth(105);

    TextBox *percentage = new TextBox(panel);
    percentage->setFixedWidth(75);
    percentage->setValue(to_string(cps_vector[0]->damping));
    percentage->setUnits("%");
    ...
    slider->setFinalCallback([&](float value) {
      for (ClothParameters *cp : cps_vector) cp->damping = (double)value;
...

  {
    ...
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(wind_amplitude.x);
    fb->setUnits("m/s^2");
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { wind_amplitude.x = value; });
    ...
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(wind_amplitude.y);
    fb->setUnits("m/s^2");
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { wind_amplitude.y = value; });
    ...
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(wind_amplitude.z);
    fb->setUnits("m/s^2");
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { wind_amplitude.z = value; });

    new Label(panel, "interval :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(wind_interval);
    fb->setUnits("m/s^2");
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { wind_interval = value; });
  }
