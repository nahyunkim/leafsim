...
void loadObjectsFromFile(string filename, vector<Cloth *> *cloths, vector<ClothParameters *> *cps, vector<CollisionObject *>* objects) {
  // Read JSON from file
  ifstream i(filename);
  json j;
  i >> j;
  ...
  for (json::iterator it = j.begin(); it != j.end(); ++it) {
    ...
    if (key.find(CLOTH) != string::npos) {
      Cloth *cloth = new Cloth();
      ClothParameters *cp = new ClothParameters();
      // Cloth
      double width, height;
      float thickness;
      e_orientation orientation;
      vector<vector<int>> pinned;
      vector<vector<double>> transform_data;
      vector<vector<int>> leaf_grid;
      ...
      auto it_transform_data = object.find("transform_data");
      if (it_transform_data != object.end()) {
        vector<json> transforms = *it_transform_data;
        for (auto matrix : transforms) {
          vector<double> m = matrix;
          transform_data.push_back(m);
        }
      }

      auto it_leaf_grid = object.find("leaf_grid");
      if (it_leaf_grid != object.end()) {
        vector<json> rows = *it_leaf_grid;
        for (auto row : rows) {
          vector<int> r = row;
          leaf_grid.push_back(r);
        }
      }

      cloth->width = width;
      cloth->height = height;
      cloth->num_width_points = leaf_grid[0].size();
      cloth->num_height_points = leaf_grid.size();
      cloth->thickness = thickness;
      cloth->orientation = orientation;
      cloth->pinned = pinned;
      cloth->transform_data = transform_data;
      cloth->leaf_grid = leaf_grid;
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) { // No arguments, default initialization
    ...
    loadObjectsFromFile(default_file_name, &cloths, &cps, &objects);
  } else {
    int c;
    ...
    while ((c = getopt (argc, argv, "f:")) != -1) {
      switch (c) {
        case 'f':
          loadObjectsFromFile(optarg, &cloths, &cps, &objects);
          break;
        default:
          usageError(argv[0]);
      }
